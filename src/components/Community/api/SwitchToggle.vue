<template v-slot:default="{ isActive }">
  <v-switch hide-details class="d-flex-inline switch-center" :color="'success'"
    :model-value="confirm_status == 1 ? true : false" @click="
      {
    ...
    <template v-slot:label>
      <v-dialog transition="dialog-top-transition">
        ...
        <template v-slot:default="{ isActive }">
          <v-card>
            <v-toolbar color="primary" title="แก้ไขข้อมูลกลุ่มวิสาหกิจชุมชน"></v-toolbar>
                    ...
                    <v-row dense>
                      <v-col cols="12" sm="12" md="6">
                        <v-card class="mx-auto" title="ข้อมูลผู้ใช้">
                          <v-container>
                            <v-text-field v-model="username" color="primary" label="ชื่อผู้ใช้" variant="underlined"
                              disabled></v-text-field>

                            <v-text-field v-model="password" color="primary" label="รหัสผ่าน" variant="underlined"
                              @input="checkPasswordMatch" :append-icon="show1 ? 'mdi-eye' : 'mdi-eye-off'"
                              @click:append="show1 = !show1" :type="show1 ? 'text' : 'password'"></v-text-field>

                            <v-text-field v-model="cfPassword" color="primary" label="ยืนยันรหัสผ่าน"
                              variant="underlined" @input="checkPasswordMatch"
                              :append-icon="show2 ? 'mdi-eye' : 'mdi-eye-off'" @click:append="show2 = !show2"
                              :type="show2 ? 'text' : 'password'"></v-text-field>

                            <v-text-field v-model="full_name" color="primary" label="ชื่อเต็ม"
                      ...
                      <v-col cols="12" sm="12" md="6">
                        <v-card class="mx-auto" title="ข้อมูลกลุ่มวิสาหกิจชุมชน">
                          <v-container>
                            <v-text-field v-model="shop_name" color="primary" label="ชื่อวิสาหกิจชุมชน"
                              variant="underlined"></v-text-field>

                            <v-text-field v-model="address" color="primary" label="ที่อยู่"
                              variant="underlined"></v-text-field>

                            <v-text-field v-model="mobile" color="primary" label="โทรศัพท์"
                              variant="underlined"></v-text-field>
