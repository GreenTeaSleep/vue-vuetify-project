    ...
    <v-data-table :headers="headers" :items="desserts" :search="search">
        ...
        <v-toolbar flat>
          ...
          <v-dialog v-model="dialog" max-width="960px">
            <v-card>
              <v-toolbar
                color="primary"
                :title="'รายละเอียดวิสาหกิจชุมชน ' + editedItem.commu_id"
              ></v-toolbar>
              ...
              <v-card-actions>
                ...
                <v-btn
                  color="blue-darken-1"
                  variant="text"
                  @click="save(editedItem.commu_id)"
                >
                  Save
                </v-btn>
              </v-card-actions>
      ...
      <template v-slot:item.actions="{ item }">
        <div v-if="item.raw.confirm_status > 0 && item.raw.confirm_status < 3">
          <v-switch
            :model-value="item.raw.confirm_status == 1 ? true : false"
            @click="changeStatus(item.raw.commu_id, item.raw.confirm_status)"
        ...
        <div v-else>
          <BTNDetail
            :commu_id="item.raw.commu_id"
            :name="item.raw.name"
            :address="item.raw.address"
            :mobile="item.raw.mobile"
            :regis_code="item.raw.regis_code"
            :comfirm_status="item.raw.confirm_status"
          />
        </div>
      </template>
    </v-data-table>
